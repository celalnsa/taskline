...
`skills/taskline-management/SKILL.md` is the source of truth for how an agent
works a task through the lifecycle; follow it instead of restating a workflow
here. Skills have two publication layers: `skills/` holds public, installable
agent contracts that `scripts/install-local.sh` links into user-level skill
directories, while `.agents/skills/` holds repository-only skills marked
`metadata.internal: true` that may depend on this checkout and must not be
installed globally.

Don't commit process or plan files to the repository. Durable multi-step plans
and stage artifacts belong in Taskline task docs, not the repository.
