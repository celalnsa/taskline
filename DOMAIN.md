...
and PR, required CI plus a server-compatible aggregate check rollup for the
latest head, a fixed review settle window, inspection of every review/comment
surface, handling of blocking findings, resolution of every review thread, and
a merge before `done`. It does not require that a review be posted. The
mechanical-change fast path may omit a dedicated Spec stage, but it does not
omit those delivery gates. The exact settle-window, finding-priority,
fast-path, and stage-artifact rules belong to
