...
required_rules = {
    "skills/taskline-management/SKILL.md": [
        ("latest head waits only for CI",
         r"latest PR head.*wait.*CI checks.*complete"),
        ("comments refresh after CI",
         r"after.*CI.*complete.*refresh.*review summaries.*review threads.*top-level PR comments"),
        ("no comments proceed without review wait",
         r"no reviews or comments.*continue.*without.*review.*wait"),
        ("new push waits for new head CI",
         r"new push.*invalidates.*earlier.*evidence.*wait.*new head.*CI"),
        ...
        ("all unprioritized findings block",
         r"unprioritized finding.*human.*automated.*fix.*reasoned rebuttal"),
...
prohibited_rules = {
    "skills/taskline-management/SKILL.md": [
        ...
        ("fixed review wait", r"10-minute|10 分钟|settle window"),
        ("review timer or clock", r"review (?:timer|clock)|latest-head clock"),
